    ...
    def print_excel_data(self):
        ...
        for rec in self.hr_excel_report_ids:
            ...
            for env_rcd in env_records:
                worksheet.write(Row, 2, env_rcd.project_id.name)
                worksheet.write(Row, 3, env_rcd.task_id.name)
                worksheet.write(Row, 4, env_rcd.name)
                worksheet.write(Row, 5, env_rcd.unit_amount)
                worksheet.write(Row, 6, str(self.start_date))
                worksheet.write(Row, 7, str(self.end_date))
                total_time += env_rcd.unit_amount
                Row += 1
            worksheet.write(Row, 5, total_time)
            Row += 3
